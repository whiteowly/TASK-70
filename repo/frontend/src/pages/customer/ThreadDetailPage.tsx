...
function ThreadDetailPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["customer-thread", threadId],
    queryFn: () => messageApi.customerThread(threadId!),
  ...
  useEffect(() => {
    if (threadId) {
      messageApi.customerMarkRead(threadId).catch(() => {});
    }
  }, [threadId]);
  ...
  const sendMut = useMutation({
    mutationFn: () => {
      ...
      return messageApi.customerSend(threadId!, body.trim(), key);
    },
    onSuccess: () => {
      setBody("");
      setSendError(null);
      queryClient.invalidateQueries({ queryKey: ["customer-thread", threadId] });
